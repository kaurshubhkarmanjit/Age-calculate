...
connectiondb = mysql.connector.connect(host="localhost",
                                user="root",passwd="SHUBHKARMAN",database="loginsystem")
...
def login():
    ...
    Label(root2, text="").pack()
    Button(root2, text="Login", bg="red", fg='blue', relief="groove", font=('arial', 12, 'bold'),command=login_verification).pack()
    Label(root2, text="")
...
def logged():
    ...
    Button(logged_message, text="Logout", bg="pink", fg='sky blue', relief="groove", font=('arial', 12, 'bold'), command=logged_destroy).pack()
...
def failed():
    ...
    Button(failed_message,text="Ok", bg="pink", fg='sky blue', relief="groove", font=('arial', 12, 'bold'), command=failed_destroy).pack()


def login_verification():
    user_verification = username_verification.get()
    pass_verification = password_verification.get()
    sql = "select * from usertable where username = shubhkarman and password = 67890"
